...
def send_data(ser, data_chunk):
    ...
    print(f"🟢 frame（{len(frame)} byte）：", frame.hex(' ', 1))
    ser.write(frame)
...
def receive_thread_func(ser):
            ...
            if data:
                ...
                while True:
                    ...
                    try:
                        ...
                        packet_count += 1
                        print(f"✅ received {packet_count} pakage: {value:.6f}")
    ...
    if len(received_floats) == TOTAL_PACKETS:
        np.save("ESAreceived_data0513.npy", np.array(received_floats, dtype=np.float32))
        print(f"🎯 complete. data in EESAreceived_data0513.npy （total {TOTAL_PACKETS} floats）")
