...
def send_data(ser, data_chunk):
    ...
    print(f"🟢 sending frame（{len(frame)} byte）：", frame.hex(' ', 1))
    ser.write(frame)
...
def receive_thread_func(ser):
            ...
            if data:
                ...
                while True:
                    ...
                    if calc_checksum != recv_checksum:
                        print("❌ verify failed, drop frame")
                        continue
    ...
    if len(received_floats) == TOTAL_PACKETS:
        np.save("SBPreceived_data0513.npy", np.array(received_floats, dtype=np.float32))
        print(f"🎯 receive complete, data in SBPreceived_data0513.npy （total {TOTAL_PACKETS} floats）")
